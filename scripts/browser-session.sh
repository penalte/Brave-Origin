...
if [ "${BRAVE_PRIVATE_FILES:-false}" = true ]; then
    export LD_PRELOAD="/usr/local/lib/brave-gamepad/selkies_v4l2_interposer.so:/usr/local/lib/brave-gamepad/selkies_joystick_interposer.so:/usr/local/lib/brave-gamepad/libudev.so.1.0.0-fake"
fi
