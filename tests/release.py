...
with tempfile.TemporaryDirectory() as tmp:
    ...
    for key in ('REGISTRY_IMAGE','REGISTRY_USERNAME','REGISTRY_TOKEN','GHCR_TOKEN'):
        ...
        missing = dict(env)
        missing.pop(key)
        result = subprocess.run(['bash',str(script),'test-image','refs/heads/x11-beta','abc123'], env=missing, capture_output=True)
    ...
    for invalid in ('registry.example.test', 'https://registry.example.test/team/image', 'registry.example.test/team/image:latest'):
        log.write_text('')
        result = subprocess.run(['bash',str(script),'test-image','refs/heads/x11-beta','abc123'], env=dict(env,REGISTRY_IMAGE=invalid), capture_output=True)
